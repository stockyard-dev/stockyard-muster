...
async function load(){var r=await fetch(A+'/attendees').then(function(r){return r.json()});items=r.attendees||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.email||'').toLowerCase().includes(q)||(i.event_name||'').toLowerCase().includes(q)||(i.ticket_type||'').toLowerCase().includes(q)});
...
if(i.event_name)h+='<span>'+esc(i.event_name)+'</span>';
if(i.ticket_type)h+='<span>'+esc(i.ticket_type)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.checked_in)h+='<span>'+i.checked_in+'</span>';
if(i.notes)h+='<span>'+esc(i.notes)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/attendees/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",email:"",event_name:"",ticket_type:"",status:"",checked_in:0,notes:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' MUSTER</h2>';
...
h+='<div class="fr"><label>Event Name</label><input id="f-event-name" value="'+esc(i.event_name)+'"></div>';
h+='<div class="fr"><label>Ticket Type</label><input id="f-ticket-type" value="'+esc(i.ticket_type)+'"></div>';
h+='<div class="fr"><label>Status</label><input id="f-status" value="'+esc(i.status)+'"></div>';
h+='<div class="fr"><label>Checked In</label><input id="f-checked-in" type="number" value="'+(i.checked_in||0)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),email:document.getElementById('f-email').value.trim(),event_name:document.getElementById('f-event-name').value.trim(),ticket_type:document.getElementById('f-ticket-type').value.trim(),status:document.getElementById('f-status').value.trim(),checked_in:parseInt(document.getElementById('f-checked-in').value)||0,notes:document.getElementById('f-notes').value.trim()};
if(editId){await fetch(A+'/attendees/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/attendees',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
